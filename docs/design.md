...
4. Outputs, no MLflow run (`report/html.py` writes the page from the health payload and the alert queue alone): `report/tables/<name>_batch<d>_alerts.csv` (rank, unit, score, account or node ids, top contributing features, only rows at or above the threshold) and `<name>_batch<d>_health.json` (status, each detector's score and flag level, counts, threshold, commit, dataset and feature versions, split hash). and `<name>_batch<d>_report.html`, one self-contained page (no scripts, nothing loaded from outside) with each detector's score beside its flag level as a figure and a table, the top 25 alerts, and the provenance. Exit 0 = no flag, 3 = a detector flagged, 1 = error.
