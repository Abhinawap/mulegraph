...
1. CLI parses `mulegraph run --config configs/elliptic_temporal.yaml`; Pydantic validates the config.
...
- Week 1 also times one SAGE fit and one PNA fit on AMLworld HI-Small using IBM's Multi-GNN repo and its own preprocessing (the paper's reported run times are the prior; expect hours for PNA), and records the dataset's real time span. The MVP has exactly one loader (Elliptic).
