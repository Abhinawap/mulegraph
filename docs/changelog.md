...
- **Changed** `lead_time` so a detector's first flag must start a `drop_run`-long run of flags, the same persistence the F1 drop already needed (PR-R4). Before, one isolated flag counted: on Elliptic PSI flagged t39 and t41, went silent through the t42–t45 collapse, then flagged t46 and t48–49, and KS never flagged two batches in a row, so the old lead of 4 / 3 timesteps rested on single flags. Rerun result (3 min 34 s, 2.9 GB RSS): PSI first holds a flag at t48 for every model and seed (XGBoost lead −5; SAGE −9 against its t39–40 dip in four seeds, −5 against t43 in the fifth); KS and the score-shift detector never hold one. README drift section, figure and status rewritten around the negative result, with the SAGE dip-versus-collapse caveat.
