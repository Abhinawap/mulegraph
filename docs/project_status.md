...
- **GFP is not causal by construction — only by usage.** It is stateful: `partial_fit` accumulates a graph, `transform` reads the accumulated state. Ingesting a whole table before transforming leaks the future backwards. Verified on a toy fixture: with all edges ingested first, a transaction at *t1* gained neighbour amount statistics generated by an edge at *t2*. The only safe drive pattern is **strictly time-ordered, batch at a time: `partial_fit(batch_t)` then `transform(batch_t)`, never a global fit first.** PR-F2 therefore constrains `features/`, not just the config, and the synthetic-fixture causality test is what guards it.
