...
**Status:** first draft for issue #8, written 16 Sep 2026 against the code at `ec4501f` (results) and the `mvp` tag on the merge commit of the MVP closeout PR (#29). The draft states definitions as the code implements them. Where the spec differs from the code, this draft follows the code. Items marked `TODO` need a source or a number that does not exist yet.
...
| Tag | `mvp`, on the merge commit of #29 (code identical to `ec4501f`; the later merges add documentation and the MLflow export only) |
