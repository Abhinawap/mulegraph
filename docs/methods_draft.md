...
**Status:** first draft for issue #8, written 16 Sep 2026 against the code at `ec4501f` (results) and the `mvp` tag on merge commit `dd85dec`. The draft states definitions as the code implements them. Where the spec differs from the code, this draft follows the code. Items marked `TODO` need a source or a number that does not exist yet.

Sources used throughout: `docs/project_spec.md` (v0.8), `docs/project_status.md` (*Verified method notes*), `docs/viability_review_2026-09.md`, `configs/elliptic_mvp.yaml`, `pyproject.toml`, `uv.lock`, `report/tables/elliptic_mvp_results.csv` and the committed MLflow export `report/exports/mvp_elliptic_mvp_runs.csv` (`d16baab`).
...
We model Elliptic++ as a transaction graph: a node is a transaction, an edge is a flow of bitcoin from one transaction's outputs to another's inputs (D1). The 49 timesteps form 49 disconnected components. The Elliptic++ actor (wallet) graph, where entities do persist across time, is out of scope until Later.

This graph structure fixes how many evaluation regimes Elliptic can support. The spec defines three (PR-E1):
...
The pipeline builds every split before the first model fit, so a config that requests the regime fails in seconds instead of after hours of fitting. The three-regime contrast belongs to AMLworld (v1b), where accounts persist across days.
...
Graph features come from IBM's Graph Feature Preprocessor, `snapml.GraphFeaturePreprocessor`, version 1.17.2 (Blanuša et al., 2024). It installed from a wheel on Python 3.11 in week-1 gate 1, so the `igraph` fallback in the spec was never built; requesting it raises an error that says so (PR-F1).
...
GFP holds a graph in memory, and `transform(batch)` inserts the batch into that graph before it scores the batch's edges. Causality therefore depends on how the caller drives it, not on anything inside GFP (PR-F2). In gate 1 we verified two ways to get it wrong on toy fixtures:
...
GFP output is deterministic: bit-identical across repeated runs and across 1 and 12 threads (gate 1).
...
`TODO(source)`: the probe was run ad hoc on 16 Sep 2026 and is not yet pinned by a test. Add it to `tests/test_features_gfp_layout.py` and cite the test here.
...
- `device` = the resolved device (CUDA for the MVP run, per the logged `device` parameter).
...
The MVP grid ran 50 fits (5 configs × 2 regimes × 5 seeds) in under nine minutes on an RTX 4060 Laptop GPU. The slowest fit was `random sage.base` at 44.3 s; the slowest XGBoost fit took 1.5 s (fit times from the MLflow export).
...
- **ROC-AUC**, reported as secondary; whether to keep it is open question 4 with the supervisor;
...
A 1,000-sample bootstrap over test ids is reserved for per-timestep curves and single-seed figures (v1a, #12). It is narrow whatever the training instability, because it resamples test nodes around one fitted model. It is never pooled with the seed interval and never used to call a gap significant (PR-E3). The MVP code has no bootstrap yet.

### 8.3 Paired gaps

Each comparison of interest (`base` against `base_gfp` within a model; XGBoost against SAGE within a feature set; `raw165` against `base_gfp`) is to be reported as a paired-by-seed difference: for each seed *s*, *d<sub>s</sub>* = metric(A, *s*) − metric(B, *s*), with the same t-interval over the five *d<sub>s</sub>*. A gap is significant only if that interval excludes zero. Paired gaps are v1a work (#11) and do not exist in the MVP code, so this chapter calls no gap significant.

### 8.4 Known issue: deterministic pairs

XGBoost trial 0 is deterministic (§6.1). For an XGBoost-versus-XGBoost gap, such as `xgb.base` against `xgb.base_gfp`, all five paired differences are identical, the paired standard deviation is zero, the t-interval collapses to a point, and any nonzero difference would be marked significant. That would be a significance claim with no variance behind it. Gaps between XGBoost and SAGE are unaffected, because SAGE seeds vary.

Issue #27 records the rule planned for v1a, fixed before any v1a gap numbers exist. The gap code will mark a pair **degenerate** when all paired differences are identical (tested as `np.ptp(diffs) == 0`, since the float mean of identical values can leave a residue of about 1e-17), write `significant` as not applicable, and record the reason (`deterministic_pair`) in the gap CSV. Adding `subsample < 1` to trial 0 to create variance is ruled out: D2 fixes trial 0 as library defaults, and changing it would move XGBoost's point estimate to make an interval behave. The bootstrap cannot stand in either (PR-E3). How the dissertation words a deterministic gap waits on the supervisor (#9). None of this is implemented yet.
...
## 9. Hyperparameter search and the wall-clock budget

### 9.1 Protocol (v1a onward)

D2 fixes the search protocol. Search runs once per (model config, regime, dataset) at a fixed search seed, and the best configuration is refitted over the five seeds. The budget is wall-clock: one cap *W* per dataset, identical for every model. Trial counts (40 for XGBoost, 20 for SAGE, 10 for PNA) are ceilings, not budgets. Trial 0 is the fixed reference configuration, so every model has a valid result even if nothing else completes inside *W*. Each search logs `wallclock_cap_min`, `wallclock_used_min`, `trial_ceiling`, `trials_completed`, `trial0_source` and `best_trial`.

*W* for a dataset is max(120 min, 2 × the slowest measured single fit on that dataset), so every model completes trial 0 and at least one searched trial.

### 9.2 What the MVP ran

The MVP ran no search. Every fit used trial 0. `search.enabled` is false in `configs/elliptic_mvp.yaml`, and the config schema refuses `true` with an error explaining that *W* is not yet set for every dataset. Every child run logs `trials_completed = 0`, an honest zero rather than a nominal 1, and `trial0_source` (`xgboost_defaults` or `sage_default_2x64`).

### 9.3 *W* on Elliptic

The slowest of the 50 MVP fits took 44.3 s, so

*W*<sub>Elliptic</sub> = max(120 min, 2 × 44.3 s) = max(120 min, 88.6 s) = **120 min**.
...
If the working tree has uncommitted changes, the pipeline stamps every run's commit as `<sha>-dirty`, and the stamp flows into the `commit` column of the results CSV. A warning on stderr disappears; the stamp stays with the numbers. The MVP results carry the clean stamp `ec4501f`.
...
### 10.4 Provenance of the MVP numbers

| Item | Value |
|---|---|
| Code that produced the results | `ec4501f` |
| Tag | `mvp`, on merge commit `dd85dec` (code identical to `ec4501f`; the merge adds documentation only) |
...
To regenerate: check out `mvp`, run `uv sync`, place the three Elliptic++ 2023.1 CSVs in `data/raw/elliptic_pp/2023.1/`, and run `uv run mulegraph run --config configs/elliptic_mvp.yaml`. A run from the tag stamps `dd85dec`, not `ec4501f`. XGBoost rows should reproduce exactly; SAGE rows reproduce within GPU nondeterminism.

## 11. Limitations

- **AMLworld is synthetic** (NFR-4). Its transactions and laundering typologies come from a simulator (Altman et al., 2023). Results on AMLworld describe the simulator's world; the dissertation makes no claim that they generalise to real bank transactions.
...
- **No search in the MVP.** Every result is trial 0. The XGBoost reference is library defaults and the SAGE reference is partly our choice; neither is tuned.
...
- **One machine.** Timings, and therefore *W*, come from one laptop GPU.

## 12. Elliptic results preview
...
`TODO(source)`: these per-window numbers come from an ad hoc refit recorded in `docs/project_status.md` and the viability review, not from a committed code path. The per-timestep curves (#12, v1a) must regenerate them before they enter the dissertation (S5).
...
The random split inflates F1 by 0.18 to 0.33 over the temporal split for every config. On both splits XGBoost scores above SAGE, `raw165` scores highest, and adding GFP features does not raise F1 for either model. These are descriptive readings of point estimates. No gap is called significant until paired intervals and the deterministic-pair rule land in v1a (#11, #27).
...
**Šafář et al. (2026)**, *The enemy of reproducibility is opacity: What's inside the Elliptic bitcoin dataset (and why it is wrong)*, FSI: Digital Investigation (DFRWS USA 2026). **Abstract only; full text pending (library access).** The abstract argues that Elliptic's feature construction is opaque and that leakage across the standard splits inflates reported performance. The claim to check in the full text is *where* the leakage sits: in the 72 aggregated features only, or in the 93 local features too. The design already keeps the aggregates out of `base` (PR-M7), so leakage confined to them would affect only `xgb.raw165` and the published baselines. If it reaches the local features, Elliptic stops serving as a benchmark dataset: it becomes the drift dataset only (the t43 event), and AMLworld carries the benchmark claims S1 and S2.

`TODO(source)`: replace this paragraph's abstract-level summary once the full text is read (project_status *What's next*, item 0b).
...
- Maganti (2026). When Graph Structure Becomes a Liability: A Critical Re-Evaluation of GNNs for Bitcoin Fraud Detection under Temporal Distribution Shift. arXiv:2604.19514. `TODO(source)`: author's full name.
- Šafář, Pluskal, Veselý and Ryšavý (2026). The enemy of reproducibility is opacity: What's inside the Elliptic bitcoin dataset (and why it is wrong). *Forensic Science International: Digital Investigation*. `TODO(source)`: first names, volume and article number from the full text.
